...
const StrukturBag = () => {
  ...
  const { data } = useQuery(GET_STRUKTUR_ORG, {
    variables: { unitKerjaId: id, bagianId: parseInt(bagId) },
  })
  ...
  if (data?.daftarStrukturOrganisasi.length > 0) {
    ...
    kepala = strOrg.find((el) => el.subbag === null && el.posisi.kategori === 1)
    const wak = strOrg.filter(
      (el) => el.bagian !== null && el.posisi.kategori === 2 && el.subbag === null,
    )
    wak.forEach((row) => {
      cardList.push(
        <CCol xs key={row.id}>
    ...
    const kepSubbag = strOrg.filter((el) => el.subbag !== null && el.posisi.kategori === 1)
    kepSubbag.forEach((row) => {
      cardList.push(
        <CCol xs key={row.id}>
          <CCard className="h-100 text-center">
            <CCardBody>
              ...
              <CButton
                ...
                onClick={() =>
                  navigate(
                    '/struktur-org/' + row.unitKerja.id + '/' + row.bagian.id + '/' + row.subbag.id,
                  )
                }
              >
                Anggota
    ...
    const ang = strOrg.find(
      (el) => el.bagian !== null && el.posisi.kategori === 3 && el.subbag === null,
    )
  ...
  return (
    <>
      ...
      <CButton className="mb-3" color="secondary" onClick={() => navigate('/struktur-org/' + id)}>
      ...
      <CCard className="text-center">
        <CCardBody>
          ...
          <CCardText>{kepala?.bagian.nama}</CCardText>
